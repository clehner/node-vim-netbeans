...
function VimClient(server, socket) {
	...
	socket.on('end', this._onDisconnected = this._onDisconnected.bind(this));
	socket.on('data', this._onData = this._onData.bind(this));
	socket.on('error', this._onError = this._onError.bind(this));
}
...
VimClient.prototype._onData = function (data) {
	...
	if (!this.authed) {
		if (this.server.httpServer && str.indexOf('HTTP/1') != -1) {
			// hand off the socket to the http server
			this._cleanup();
			this.server.httpServer.emit('connection', this.socket);
			this.socket.emit('data', data, 0, data.length);
			return;
		}
	}
...
VimClient.prototype._onMessage = function (message) {
	...
	// Handle the special AUTH message
		...
		this.server._authClient(this, pass);
		this.authed = true;

	} else {
		if (this.debug) console.log("message from unauthed client discarded");
		this.socket.end();
	}
};
...
VimClient.prototype._cleanup = function () {
	...
	this.server._cleanupClient(this);
	this.removeAllListeners();
	this.socket.removeListener('end', this._onDisconnected);
